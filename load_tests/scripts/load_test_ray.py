#!/usr/bin/env python3
"""
Load test script for Ray-enabled FastAPI server (api_ray.py)
Tests concurrent user performance with Ray.
"""
...
class RayLoadTester:
    """Load tester for Ray-enabled API performance comparison."""
                ...
                with open(image_path, 'rb') as f:
                    ...
                    async with session.post(f"{self.base_url}/{endpoint}", data=data) as response:
                        ...
                        return {
                            ...
                            'ray_worker_id': result.get('ray_worker_id') if response.status == 200 else None
                        }
            else:
                # Simple GET request
    ...
    async def run_load_test(self, num_users: int, requests_per_user: int, 
        ...
        """Run the load test."""
        print(f"🚀 Starting Ray-enabled load test with {num_users} users, {requests_per_user} requests per user")
        ...
        # Analyze Ray worker usage
        ray_worker_ids = [r.get('ray_worker_id') for r in successful_requests if r.get('ray_worker_id')]
        unique_workers = set(ray_worker_ids)
        ...
        stats = {
            ...
            'ray_workers_used': len(unique_workers),
            'unique_ray_workers': list(unique_workers),
            'results': all_results
        }
    ...
    def print_results(self, stats: Dict[str, Any], api_type: str):
        ...
        if 'ray_workers_used' in stats:
            print(f"Ray Workers Used: {stats['ray_workers_used']}")
            print(f"Unique Ray Workers: {stats['unique_ray_workers']}")
...
async def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Load test for Ray-enabled API performance')
    ...
    tester.print_results(stats, "ray")
    tester.save_results(stats, "ray")
